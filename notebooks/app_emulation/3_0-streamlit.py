...
import joblib
import pandas as pd
from config import COMBUSTION_MODEL_FILE, ELECTRIC_COLS_VALS_SELECTION_FILE, ELECTRIC_MODEL_FILE, ELECTRIC_PREDICTION_PRESELECT_FILE, SCALER_COMBUSTION_FILE, SCALER_ELECTRIC_FILE, TRAIN_TEST_SPLIT_COMBUSTION_FILE, TRAIN_TEST_SPLIT_ELECTRIC_FILE, RAW_DATA_COMBUSTION_FILE
...
if model_type == sidebar_sel_pred_test_electric:
    ...
    with open('files/combustion-encoder.pkl', 'rb') as f:
        encoders, cat_cols = pickle.load(f)
    with open('files/raw-dataset_combustion.pkl', 'rb') as f:
       df_pe_cleaned = pickle.load(f)
    
    # Pick the columns you want to show in the multiselect label
    # Selectin fields for the columns      
    # unique values of columns
    uniques_m_s = df_pe_cleaned['member_state'].unique()
    choice_m_s= st.selectbox('Member_state:', uniques_m_s)
    uniques_m_n_e = df_pe_cleaned['manufacturer_name_eu'].unique()
    choice_m_n_e= st.selectbox('Manufactorer_EU:', uniques_m_n_e)
    uniques_v_t = df_pe_cleaned['vehicle_type'].unique()
    choice_v_t= st.selectbox('vehicle_type:', uniques_v_t)
    uniques_c_o_v = df_pe_cleaned['category_of_vehicle'].unique()
    choice_c_o_v= st.selectbox('category of vehicle:', uniques_c_o_v)
    uniques_f_t = df_pe_cleaned['fuel_type'].unique()
    choice_f_t= st.selectbox('fuel_type:', uniques_f_t)
    uniques_f_m = df_pe_cleaned['fuel_mode'].unique()
    choice_f_m= st.selectbox('fuel_mode:', uniques_f_m)
    uniques_weltp_test_mass = df_pe_cleaned['weltp_test_mass'].unique()
    choice_weltp_test_mass= st.selectbox('wltp_test_mass [kg]):', uniques_weltp_test_mass)
    choice_weltp_test_mass=round(float(choice_weltp_test_mass), 1)
    # Field for free entry
    custom_value = st.text_input('Free input weltp_test_mass [kg]', key = 'weltp_test_mass')
    if custom_value:
        choice_weltp_test_mass = custom_value
        choice_weltp_test_mass=round(float(choice_weltp_test_mass), 1)   
# Überprüfen, ob ein eigener Wert eingegeben wurde
    uniques_e_c = df_pe_cleaned['engine_capacity'].unique()
    choice_e_c= st.selectbox("engine_capacity [cm^3] :", uniques_e_c)
    choice_e_c=round(float(choice_e_c), 1)
    custom_value = st.text_input('Free input engine capacity [cm^3]', key= 'engine_capacity')
    if custom_value:
        choice_e_c = custom_value
        choice_e_c=round(float(choice_e_c), 1)   
    uniques_e_p = df_pe_cleaned['engine_power'].unique()
    choice_e_p= st.selectbox('engine_power [kW]', uniques_e_p)
    choice_e_p=round(float(choice_e_p), 1)
    custom_value = st.text_input('Free input engine power [kW]', key = 'engine_power')
    if custom_value:
        choice_e_p = custom_value
        choice_e_p=round(float(choice_e_p), 1)
    uniques_year = df_pe_cleaned['year'].unique()
    choice_year= st.selectbox('year:', uniques_year)
    choice_year=round(float(choice_year), 0)
    custom_value = st.text_input('Free input year', key = 'year')
    if custom_value:
        choice_year = custom_value
        choice_year=round(float(choice_year), 0)
    #writing selected values to a dataframe
    cols= ['member_state', 'manufacturer_name_eu', 'vehicle_type', 'category_of_vehicle', 'fuel_type', 'fuel_mode', 'weltp_test_mass', 'engine_capacity', 'engine_power', 'year']
    ...
    if st.button("Predict", type="primary"):
        ...
        df_pe_cleaned=df_pe_cleaned.drop(columns = "specific_co2_emissions")
        df_prediction_comb = pd.concat([df_pe_cleaned, df_prediction_comb], axis=0)
        st.write(df_prediction_comb.iloc[[-1]])
        #labelencoding the inserted values
        from sklearn.preprocessing import LabelEncoder
        enc = LabelEncoder()
        for col in cat_cols:
            df_prediction_comb[col] = enc.fit_transform(df_prediction_comb[col])
        num_cols = ["weltp_test_mass", "engine_capacity", "engine_power", "year"]
        df_prediction_comb[num_cols]= combustion_scaler.transform(df_prediction_comb[num_cols])
        y_pred = combustion_model.predict(df_prediction_comb.iloc[[-1]]) 
