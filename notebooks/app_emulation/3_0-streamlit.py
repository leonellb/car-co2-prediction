...
if model_type == sidebar_sel_pred_test_electric:
    ...
    X_train_comb, X_test_comb, y_train_comb, y_test_comb = load_test_train_split(TRAIN_TEST_SPLIT_COMBUSTION_FILE)
    combustion_model = load('files/output/models/combustion_model.joblib')


    #combustion_model = XGBRegressor(n_estimators = 1000, learning_rate = 0.3)
    #combustion_model.fit(X_train_comb, y_train_comb)  
    # Saving the model
    #models_pkl_file = "files/output/models/combustion-model.pkl"
    #with open(models_pkl_file, "wb") as f:
    #    pickle.dump(combustion_model, f)
