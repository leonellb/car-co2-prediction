...
import joblib
import pandas as pd
from config import COMBUSTION_MODEL_FILE, ELECTRIC_COLS_VALS_SELECTION_FILE, ELECTRIC_MODEL_FILE, ELECTRIC_PREDICTION_PRESELECT_FILE, SCALER_ELECTRIC_FILE, TRAIN_TEST_SPLIT_COMBUSTION_FILE, TRAIN_TEST_SPLIT_ELECTRIC_FILE, RAW_DATA_COMBUSTION_FILE
...
if model_type == sidebar_sel_pred_test_electric:
    ...
    if st.button("Predict", type="primary"):
        ...
        df_pe_cleaned=df_pe_cleaned.drop(columns = ["specific_co2_emissions", "fuel_consumption"])
        ...
        cat_cols = X_test_comb.select_dtypes(include="object").columns
        num_cols = X_test_comb.select_dtypes(include=["float64", "int64"]).columns
        for col in cat_cols:
            df_prediction_comb[col] = enc.fit_transform(df_prediction_comb[col])
        df_prediction_comb[num_cols]= scaler.fit_transform(df_prediction_comb[num_cols])
